           ...
           // add ll
            if (cumulative_catch[n] == 0){
                ...
                eta = beta[1]+v_trip[tripID[n], 1]+v_comp[compID[n], 1] + v_id[partID[n], 1] +
                          ...
                          // (beta[7]+v_trip[tripID[n], 7]+v_comp[compID[n], 7] + v_id[partID[n], 7])*success_density[n]+
                          // (beta[8]+v_trip[tripID[n], 8]+v_comp[compID[n], 8] + v_id[partID[n], 8])*loss_density[n];
...
parameters {
  ...
  // trip level offsets
  matrix[5, Ntrips] z_trip;           //Matrix for our latent individual samples (z scores)
  vector<lower = 0>[5] sigma_trip;  //Standard deviation of learning parameters among individuals
  cholesky_factor_corr[5] Rho_trip; //Cholesky factor for covariance of learning parameters among individuals
  
  // lake level offsets
  matrix[5, Ncomp] z_comp;           //Matrix for our latent individual samples (z scores)
  vector<lower = 0>[5] sigma_comp;  //Standard deviation of learning parameters among individuals
  cholesky_factor_corr[5] Rho_comp; //Cholesky factor for covariance of learning parameters among individuals
  
  // id level offsets
  matrix[5, Nids] z_id;           //Matrix for our latent individual samples (z scores)
  vector<lower = 0>[5] sigma_id;  //Standard deviation of learning parameters among individuals
  cholesky_factor_corr[5] Rho_id; //Cholesky factor for covariance of learning parameters among individuals
}

transformed parameters {

  // non centered parameterization
  matrix[Ntrips, 5] v_trip;
  v_trip = (diag_pre_multiply(sigma_trip, Rho_trip) * z_trip)';

  // non centered parameterization
  matrix[Ncomp, 5] v_comp;
